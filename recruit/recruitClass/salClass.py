...
class salClass(Controller):
    ...
    def get_upload(self):
        ...
        if file and file is not None:
            ...
            try:
                ...
                if "cannot be null" in str(e):
                    ...
                    verbose_name = RecruitSal._meta.get_field(field).verbose_name
        ...
        if createData is not None and createData != "":
            ...
            try:
                createData['recruit_sal_date'] = datetime.strptime(createData['recruit_sal_date'][0:7], "%Y-%m")

            except Exception as e:
                createData['recruit_sal_date'] = datetime.strptime(createData['recruit_sal_date'], "%Y-%m")
                # print(e)
            ...
            obj = RecruitSal.objects.create(**createData)
        ...
        for i in range(2, table.max_row):
            ...
            if table.cell(i + 1, 1).value != "" and table.cell(i + 1, 1).value is not None:
                ...
                if employee_base_father.id not in self.user_base or employee_base_son.id not in self.user_base:
                    ...
                    data_kwargs = {
                        "recruit_sal_base": employee_base_son,
                        "recruit_sal_date": datetime.strptime(table.cell(1, 1).value, "%Y-%m"),
                        "recruit_sal_demand_no": table.cell(i + 1, 3).value,
                        "recruit_sal_interview_no": table.cell(i + 1, 4).value,
                        "recruit_sal_interview_pass_no": table.cell(i + 1, 5).value,
                        "recruit_sal_offer_no": table.cell(i + 1, 6).value,
                        "recruit_sal_entry_no": table.cell(i + 1, 7).value,
                        "recruit_sal_to_entry_no": table.cell(i + 1, 8).value,
                        "recruit_sal_completion_rate": round((int(table.cell(i + 1, 8).value)+int(table.cell(i + 1, 7).value))/int(table.cell(i + 1, 3).value), 2),
                        "recruit_sal_remark": table.cell(i + 1, 10).value,
                        "recruit_sal_status": 1,
                        "creator": AdminUser.objects.filter(pk=self.check_token)[0],
                        "modifier": AdminUser.objects.filter(pk=self.check_token)[0],
                    }
                    RecruitSal.objects.create(**data_kwargs)
    ...
    def download_file(self):
        ...
        kwargs = {
            'recruit_sal_status': 1,
            "recruit_sal_base__in": self.user_base}
        ...
        obj = RecruitSal.objects.filter(**kwargs).order_by("create_time")
        if len(obj) > 0:
            obj = RecruitSalPutSerializers(instance=obj, many=True).data
            ...
            fields["中心/事业部"] = "recruit_sal_base_father"
            ws.append(row2)
