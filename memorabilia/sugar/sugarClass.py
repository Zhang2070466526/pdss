        ...
        if self.request.FILES.getlist('file'):   #上传文件
            ...
            for file_obj in self.request.FILES.getlist('file'):  # 上传文件
                ...
                for i in range(2, table.max_row):
                    ...
                    try:
                        if table.cell(i + 1, 2).value != '润阳新能源':
                            father = {
                                'name': table.cell(i + 1, 2).value,  #
                                'status': True,
                            }
                            son = {
                                'name': table.cell(i + 1, 3).value,  #
                                'status': True,
                            }
                            k = {
                                'name': table.cell(i + 1, 3).value,  #
                            ...
                            memorabilia_base_id= center_base.objects.filter(**k).values_list('id', flat=True)[
                                0]
                        else:
                            father = {
                                'name': table.cell(i + 1, 2).value,  #
                            ...
                            memorabilia_base_id = center_base.objects.filter(**father).values_list('id', flat=True)[
                                0]
            ...
            for i in all:
                ...
                if i[1] != '润阳新能源':
                    father_name = center_base.objects.filter(
                        id=center_base.objects.filter(id=i[0], status=True).values_list('base_parent_id', flat=True)[0], status=True).values_list('name', flat=True)[0]
                    ...
                    if i[1] != '润阳新能源':
                        ...
                        father_name = center_base.objects.filter(
                            ...
                            center_base.objects.filter(id=i[0], status=True).values_list('base_parent_id', flat=True)[
                                0], status=True).values_list('name', flat=True)[0]
                        i.insert(0, father_name)
                    else:
                        i.insert(0, '润阳新能源')
                        i[2] = ''
